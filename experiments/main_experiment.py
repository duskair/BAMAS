    ...
    while True:
        ...
        try:
            ...
            if not candidate_agents:
                ...
                with torch.no_grad():
                    predictor = execution_engine.get_difficulty_predictor("gsm8k")
                ...
                if difficulty:
                    ...
                    if filtered:
                        ...
                        print(f"\n[*] Difficulty->Tier: difficulty {difficulty_score:.4f} -> tiers {allowed_tiers} ({len(filtered)} candidate agents)")
                    else:
                        print(f"\n[*] Difficulty->Tier: no candidates in tiers {allowed_tiers}; keeping full pool")
                    print(f"[*] Difficulty->Tier mode -> topology {selected_pattern['name']}")
                else:
                    print(f"\n[*] Difficulty recorded (baseline, no effect) -> Difficulty: {difficulty_score:.4f}")
...
def main():
    ...
    print(f"\nPattern Selection Frequency by Budget:")
